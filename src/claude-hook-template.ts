export function claudeHookTemplate(sidecarUrl: string, dataDir: string): string {
...
// Managed by dcache for Claude Code. Do not edit by hand.
// dcache-managed-claude-hook
...
const eventName = String(input?.hook_event_name || input?.event || "unknown");
const sessionID = extractSessionID(input);
const project = projectIdentity(extractProjectPath(input) || process.cwd());
...
function record(value) {
  try {
    ...
    // Local telemetry is best effort; Claude Code must continue.
  }
}
...
function cacheAnchor() {
  ...
  try {
    ...
    // Anchor loading is best effort; Claude Code must continue.
  }
  return "";
}
...
function reconcileAnchorContext(input) {
  ...
  if (state.enabled === true) {
    ...
    if (!reason && input.eventName === "SessionStart" && hook.source === "clear") {
      reason = "claude_clear_session";
    }
    if (!reason && input.eventName === "SessionStart" && isExplicitClaudeFork(hook, state, sessionID)) {
      reason = "claude_fork_session";
    }
  }
  if (state.enabled !== true) return;
  if (reason) {
...
function isExplicitClaudeFork(input, state, sessionID) {
  return (
    (input.fork_session === true || input.forkSession === true || input.source === "fork") &&
    Boolean(sessionID) &&
...
function writeAnchorState(state) {
  try {
    ...
    // State persistence is best effort; Claude Code must continue.
  }
}
